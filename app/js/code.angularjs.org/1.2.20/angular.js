    ...
    if (cssClasses && element.setAttribute) {
      forEach(cssClasses.split(' '), function (cssClass) {
        element.setAttribute(
          'class',
          trim((' ' + (element.getAttribute('class') || '') + ' ').replace(/[\n\t]/g, ' ').replace(' ' + trim(cssClass) + ' ', ' '))
        )
      })
    }
  }
  ...
  forEach(
    {
      ...
      text: (function () {
        ...
        if (msie < 9) {
          ...
          NODE_TYPE_TEXT_PROPERTY[1] = /** Element **/ NODE_TYPE_TEXT_PROPERTY[3] = 'textContent' /** Text **/
        }
        getText.$dv = ''
        return getText
    ...
    this.$get = [
      ...
      function ($injector, $interpolate, $exceptionHandler, $http, $templateCache, $parse, $controller, $rootScope, $document, $sce, $animate, $$sanitizeUri) {
        ...
        function compileTemplateUrl(directives, $compileNode, tAttrs, $rootElement, childTranscludeFn, preLinkFns, postLinkFns, previousCompileContext) {
          ...
          $http
            .get($sce.getTrustedResourceUrl(templateUrl), { cache: $templateCache })
